...
let velocity = 0;
let score = 0;
let highScore = localStorage.getItem('flappyOwlRecord') ? parseInt(localStorage.getItem('flappyOwlRecord')) : 0;
...
function draw() {
    ...
    if (!showStartScreen) {
        ...
        if (images.layer7 && images.layer7.complete) {
            ...
            ctx.drawImage(images.layer7, CANVAS_WIDTH - offset, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
        }
...
function drawSpider(x, y) {
    if (images.spider && images.spider.complete) {
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.arc(centerX, centerY - SPIDER_SIZE / 4, SPIDER_SIZE / 5, 0, Math.PI * 2);
        ctx.fill();
...
function drawOwl() {
    if (!images.owlSprite || !images.owlSprite.complete) {
        ...
        ctx.beginPath();
        ctx.arc(OWL_X + OWL_WIDTH/2, owlY + OWL_HEIGHT/3, OWL_WIDTH / 8, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#000';
        ctx.font = '20px Arial';
        ctx.textAlign = 'center';
...
function drawUI() {
    ...
    if (showGameOver) {
        ...
        buttons = [
            new Button('JUGAR DE NUEVO', startX, 440, buttonWidth, buttonHeight, resetGame),
            new Button('VOLVER AL INICIO', startX + buttonWidth + buttonGap, 440, buttonWidth, buttonHeight, returnToStartScreen, '#808080')
        ];
...
function checkCollision(owlX, owlY, owlW, owlH, obstacle) {
    ...
    return isOverlapping(owlRect, topPipeRect) || isOverlapping(owlRect, bottomPipeRect);
}
...
function checkSpiderCollision(owlX, owlY, spider) {
    ...
    return owlRect.x < spiderRect.x + spiderRect.width &&
...
canvas.addEventListener("mousemove", (e) => {
    ...
    if (needsRedraw && (!gameRunning || showStartScreen || showGameOver || isExploding)) {
        draw();
    }
});
